...
public class DeleteStudentServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        TypedQuery<StudentEntity> query = em.createQuery("delete from StudentEntity s  where s.id=:cid", StudentEntity.class);
        ...
        try {
            ...
            query.executeUpdate();
            transaction.commit();
            responseText.append("<h3>!!!<i>Stergere executata cu succes</i>!!!</h3>");
            responseText.append("<h2>Lista actualizata studenti</h2>");
        } catch (TransactionRequiredException ex) {
            ex.printStackTrace();
            responseText.append("<h3>!!!<i>Stergere ESUATA</i>!!!</h3>");
